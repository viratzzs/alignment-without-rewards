"""
Evaluation script for the test split of lucasmccabe/logiqa.

Uses vLLM for fast batched generation.

Usage:
    python run_test.py <model_name_or_path> [--max-samples N] [--max-tokens 1024] [--batch-size 50]

Example:
    python run_test.py Qwen/Qwen3.5-4B
    python run_test.py ./outputs/Qwen3.5-4B-RL/final_model --max-samples 100
"""
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate a HuggingFace model on LogiQA test split")
    ...
    log_file = os.path.join(log_dir, f"{safe_model_name}_test.log")
    logger.add(log_file, mode="w")
    logger.info(f"Logging evaluation results to {log_file}")

    # ── Load dataset ──────────────────────────────────────────────────────
    logger.info(f"Loading test split of lucasmccabe/logiqa...")
    ...
    for idx in range(len(data)):
        ...
        # Print first few and ANY examples that extracted an incorrect answer
        ...
        if idx < 3 or is_extracted_but_wrong:
            ...
            logger.info(f"Question: {sample['query'][:200]}...")
